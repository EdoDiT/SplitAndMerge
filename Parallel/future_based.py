...
class FutureSplitAndMerge:
    ...
    def build_quadtree(self):
        """Parallel version: Builds a quadtree from the image by recursively splitting it into homogeneous blocks."""
        ...
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(process_node, self.quadtree)]
            while futures:
                ...
                for future in done:
                    ...
                    if result:
                        for child in result:
                            new_futures.append(executor.submit(process_node, child))
                futures = new_futures
