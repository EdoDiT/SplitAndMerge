...
class SplitAndMerge:
    ...
    """
    def __init__(self, image: np.ndarray, min_th:int, max_th:int, min_dim:int):
        """
    ...
    def extract_statistical_info(self):
        ...
        threshold_image = np.zeros_like(self.image)
        threshold_image[(self.image >= self.min_th) & (self.image <= self.max_th)] = 255  # Setting pixels in the ROI to white

        bone_pixels = self.image[threshold_image > 0]
    ...
    def split_and_merge(self):
        ...
        segmented_image = np.zeros_like(self.image, dtype=np.int32)
        ...
        recursive_split_and_merge(0, 0, self.image.shape[0])
